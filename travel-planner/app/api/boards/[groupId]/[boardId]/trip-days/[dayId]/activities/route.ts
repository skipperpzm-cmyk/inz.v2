...
export async function POST(request: Request, context: Params) {
  ...
  try {
    ...
    const inserted = await sqlClient`
      insert into public.board_trip_activities (day_id, time, title, description, cost)
      values (${dayId}::uuid, ${time}, ${title}, ${description}, ${cost})
      returning id
    `;

    const row = Array.isArray(inserted) && inserted.length ? inserted[0] : null;
    if (!row) return NextResponse.json({ error: 'Failed to create activity' }, { status: 500 });

    return NextResponse.json({ success: true, activityId: String((row as any).id) });
  } catch (err) {
    console.error('trip activity create error', err);
