...
type Params = { params: Promise<{ groupId: string; boardId: string; dayId: string }> };
...
async function dayBelongsToBoard(dayId: string, boardId: string) {
  if (!sqlClient) return false;
  const rows = await sqlClient`
    select 1
    from public.board_trip_days
    where id = ${dayId}::uuid and board_id = ${boardId}::uuid
    limit 1
  `;
...
export async function PATCH(request: Request, context: Params) {
  ...
  const { groupId, boardId, dayId } = await context.params;
  if (!groupId || !boardId || !dayId) return NextResponse.json({ error: 'Missing route params' }, { status: 400 });
  if (!UUID_RE.test(dayId)) return NextResponse.json({ error: 'Invalid day id' }, { status: 400 });
  ...
  try {
    ...
    const belongs = await dayBelongsToBoard(dayId, access.boardId);
    if (!belongs) return NextResponse.json({ error: 'Trip day not found' }, { status: 404 });
    ...
    const rows = await sqlClient`
      ...
      set
        ...
        updated_at = now()
      where id = ${dayId}::uuid
        and board_id = ${access.boardId}::uuid
      returning id
    `;
    ...
    console.error('trip day update error', err);
    return NextResponse.json({ error: 'Failed to update trip day' }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, context: Params) {
  ...
  try {
    ...
    const deleted = await sqlClient`
      delete from public.board_trip_days
      where id = ${dayId}::uuid
        and board_id = ${access.boardId}::uuid
      returning id
    `;
    ...
    await sqlClient`select public.reindex_board_trip_days(${access.boardId}::uuid)`;

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('trip day delete error', err);
    return NextResponse.json({ error: 'Failed to delete trip day' }, { status: 500 });
  }
}
