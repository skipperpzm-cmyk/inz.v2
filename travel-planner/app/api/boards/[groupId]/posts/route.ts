...
async function resolveMembership(userId: string, groupIdOrSlug: string) {
...
export async function GET(_request: Request, context: Params) {
  ...
  try {
    const member = await resolveMembership(userId, groupId);
    ...
    const postRows = await client`
      ...
      where gp.group_id = ${member.group_id}
        and (
...
export async function POST(request: Request, context: Params) {
  ...
  try {
    ...
    const inserted = await client`
      insert into public.group_posts (group_id, author_id, content)
      values (${member.group_id}, ${userId}, ${content})
      returning id, group_id, author_id, content, created_at
    `;
