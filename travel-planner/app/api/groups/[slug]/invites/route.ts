import { NextRequest, NextResponse } from "next/server";
import { getCurrentUser } from "@/lib/auth";
import { requireDb, sqlClient } from "@/src/db/db";

async function findGroup(db: any, key: string) {
  const res = await db`
    select id from public.groups where id::text = ${key} or slug = ${key} limit 1
  `;
  const rows = res;
  return Array.isArray(rows) && rows.length ? rows[0] : null;
}

export async function POST(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Not authenticated" }, { status: 401 });

  const params = await context.params;
  const slug = params.slug;
  console.log("[group invite] slug przekazany do backendu:", slug);
  if (!slug) return NextResponse.json({ error: "Missing slug" }, { status: 400 });

  let body: any = {};
  try { body = await request.json(); } catch { /* ignore */ }
  const userIds: string[] = Array.isArray(body?.userIds) ? body.userIds : [];
  if (userIds.length === 0) return NextResponse.json({ error: "Missing userIds" }, { status: 400 });

  try {
    const group = await findGroup(sqlClient, slug);
    if (!group) return NextResponse.json({ error: "Group not found" }, { status: 404 });

    const adminRows = await sqlClient`
      select 1 from public.group_members where group_id = ${group.id} and user_id = ${user.id} and role = 'admin' limit 1
    `;
    if (!Array.isArray(adminRows) || adminRows.length === 0) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    let invited = 0;
    for (const targetId of userIds) {
      if (!targetId) continue;
      // Ensure target is a friend
      const friendRows = await sqlClient`
        select 1 from public.user_friends where user_id = ${user.id} and friend_id = ${targetId} limit 1
      `;
      if (!Array.isArray(friendRows) || friendRows.length === 0) continue;

      // Skip if already member
      const memRows = await sqlClient`
        select 1 from public.group_members where group_id = ${group.id} and user_id = ${targetId} limit 1
      `;
      if (Array.isArray(memRows) && memRows.length) continue;

      // Insert invite (ignore duplicates)
      await sqlClient`
        insert into public.group_invites (group_id, from_user_id, to_user_id, status, created_at)
        values (${group.id}, ${user.id}, ${targetId}, 'pending', now())
        on conflict do nothing
      `;
      invited += 1;
    }

    return NextResponse.json({ invited });
  } catch (err) {
    console.error("group invite create error", err);
    return NextResponse.json({ error: "Failed to invite members" }, { status: 500 });
  }
}
